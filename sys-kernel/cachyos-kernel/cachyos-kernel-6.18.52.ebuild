...
# Source tag is cachyos-6.18.52-1; downstream scheduler rebases remain pinned
# to immutable overlay bytes.
CACHYOS_PR="$(( ${PR#r} + 1 ))"
...
# PREEMPT_RT, and RT-BORE. Its moving 6.18 kernel-patches inputs are stale for
# cachyos-6.18.52-1, so commit-pinned downstream rebases preserve those
# advertised variants. The hardened rebase needs one context-only adjustment
# for the 6.18.52 inode_get_ctime_sec() accessor conversion.
...
src_prepare() {
	...
	local hardened_patch="${T}/${CACHYOS_REBASE_PREFIX}-hardened.patch"
	...
	if use cachyos-hardened; then
		[[ $(grep -Fxc $' \tstat->ctime.tv_sec = inode->i_ctime_sec;' \
			"${rebases_prefix}-hardened.patch") -eq 1 ]] ||
		...
		sed $'s/^ \tstat->ctime.tv_sec = inode->i_ctime_sec;$/ \tstat->ctime.tv_sec = inode_get_ctime_sec(inode);/' \
			"${rebases_prefix}-hardened.patch" > "${hardened_patch}" || die
		eapply --fuzz=0 -- "${hardened_patch}"
