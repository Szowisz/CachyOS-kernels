...
src_install() {
	...
	# /usr/lib helpers
	exeinto /usr/lib
	doexe "${S}/usr/lib/iw-set-regdomain"

	# systemd unit
	systemd_dounit \
		"${S}/usr/lib/systemd/system/pci-latency.service" \
		"${S}/usr/lib/systemd/system/cachyos-iw-set-regdomain.path" \
		"${S}/usr/lib/systemd/system/cachyos-iw-set-regdomain.service"
	...
	# systemd slice drop-ins
	local slicedir
	for slicedir in session.slice.d system.slice.d user-.slice.d; do
		insinto "$(systemd_get_systemunitdir)/${slicedir}"
		doins "${S}/usr/lib/systemd/system/${slicedir}"/*.conf
	done
