...
ADDITIONAL_UPSTREAM_PATCH_VERSIONS="6.12.62-63"
...
src_prepare() {
	...
	# Fix BMQ scheduler patch for kernel 6.12.52+ compatibility
	# These kernels changed sg_cpu->sg_policy->limits_changed to use WRITE_ONCE()
	# Create a temporary fixed version of the patch without modifying the original
	local bmq_patch="${files_dir}/sched/0001-prjc-cachy.patch"
	local bmq_patch_fixed="${T}/0001-prjc-cachy-fixed.patch"
	if [[ -f "${bmq_patch}" ]]; then
		sed 's/sg_cpu->sg_policy->limits_changed = true;/WRITE_ONCE(sg_cpu->sg_policy->limits_changed, true);/g' \
			"${bmq_patch}" > "${bmq_patch_fixed}" || die "Failed to fix BMQ patch"
	fi
