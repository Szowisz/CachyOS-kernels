...
REQUIRED_USE="
	...
	?? ( o3 os )
	^^ ( hugepage_always hugepage_madvise )
	?? ( auto-cpu-optimization mgeneric mnative_amd mnative_intel mk8 mk8sse3 mk10 mbarcelona mbobcat mjaguar mbulldozer mpiledriver msteamroller mexcavator mzen mzen2 mzen3 mzen4 mmpsc matom mcore2 mnehalem mwestmere msilvermont msandybridge mivybridge mhaswell mbroadwell mskylake mskylakex mcannonlake micelake mgoldmont mgoldmontplus mcascadelake mcooperlake mtigerlake msapphirerapids mrocketlake malderlake )
"
...
src_prepare() {
	...
	if use rt-bore; then
		scripts/config -e SCHED_BORE --set-val MIN_BASE_SLICE_NS 1000000 -e PREEMPT_COUNT -e PREEMPTION -d PREEMPT_VOLUNTARY -d PREEMPT -d PREEMPT_NONE -d PREEMPT_RT -d PREEMPT_DYNAMIC -e PREEMPT_BUILD -e PREEMPT_BUILD_AUTO -e PREEMPT_AUTO || die
	fi
	...
	for MARCH in "${march_list[@]}"; do
		if use "${MARCH}"; then
			...
			MARCH_UPPER=$(echo "$MARCH_TRIMMED" | tr '[:lower:]' '[:upper:]')
			if [[ ! "$MARCH_UPPER" =~ GENERIC* ]]; then
				MARCH_UPPER="M${MARCH_UPPER}"
			else
...
# 26bbb1b846b27e4dfd5bc8eac5be2f562ed28cdd
