...
# Gentoo has not added gentoo-sources-7.1.0 yet. genpatches-7.1-1 is
# the matching 7.1 base patchset, but it has no 1000_linux-7.1.0.patch,
# so kernel-2's genpatches sanity check would otherwise fail.
...
src_unpack() {
	...
	# CachyOS tarball already includes the rxrpc skb unshare fix from genpatches.
	UNIPATCH_EXCLUDE+=" 2405"

	# CachyOS tarball already includes the skb shared frag marker fix from genpatches.
	UNIPATCH_EXCLUDE+=" 1500_net-skbuff-prop-shared-frag-marker-through-pskb-copy.patch"
...
src_prepare() {
	...
	if use rt; then
		# PREEMPT_RT is in the 7.1.0 source tree and enabled below.
		# kernel-patches/7.1 currently does not ship misc/0001-rt-i915.patch.
...
# 3d40b72fc3c40581269f9e7a12433950f2fd6d95
