...
src_unpack() {
	...
	use zfs && (cp $FILESDIR/kernel-build.sh zfs/ || die)
}
...
src_prepare() {
	...
	if use rt-bore; then
		scripts/config -e SCHED_BORE -e PREEMPT_COUNT -e PREEMPTION -d PREEMPT_VOLUNTARY -d PREEMPT -d PREEMPT_NONE -d PREEMPT_RT -d PREEMPT_DYNAMIC -e PREEMPT_BUILD -e PREEMPT_BUILD_AUTO -e PREEMPT_AUTO || die
	fi
...
pkg_postinst() {
	...
	use zfs && ewarn "ZFS support build way: https://github.com/CachyOS/linux-cachyos/blob/f843b48b52fb52c00f76b7d29f70ba1eb2b4cc06/linux-cachyos-server/PKGBUILD#L573, and you can check linux/zfs/kernel-build.sh as example"
...
# c6d6a8f9f3a4947eb7453104947186e26261d840
